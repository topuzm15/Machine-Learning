...
#Todo resimler üzerinde yapılacak oynamalar
...
def pre_process(images):
    images = tf.map_fn(lambda image: proces_image(image),images)
    return images

with tf.device('/cpu:0'):
    disorted_images = pre_process(x)
...
def fc_layer(input, size_in, size_out, relu=True, dpout = True):
...
conv1 = conv_layer(disorted_images, 3, 32, use_pooling=True)
...
logits = fc_layer(fc2, 256, 10, relu=False,dpout=False)
